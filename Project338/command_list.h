...
public:
	...
	std::shared_ptr<Scene> LoadSceneFromFile(const std::wstring& file_name, const std::function<bool(float)>& loading_progres = std::function<bool(float)>());
	std::shared_ptr<Scene> LoadSceneFromString(const std::string& scene_string, const std::string& format);

	std::shared_ptr<Scene> CreateCube(float size = 1.0, bool reverse_winding = false);
	std::shared_ptr<Scene> CreateSphere(float radius = 0.5f, uint32_t tessellation = 16, bool revers_winding = false);
	std::shared_ptr<Scene> CreateCylinder(float radius = 0.5f, float height = 1.0f, uint32_t tessellation = 32, bool reverse_winding = false);
	std::shared_ptr<Scene> CreateCone(float radius = 0.5f, float height = 1.0f, uint32_t tessellation = 32, bool reverse_winding = false);
	std::shared_ptr<Scene> CreateTorus(float radius = 0.5f, float thickness = 0.333f, uint32_t tessellation = 32, bool reverse_winding = false);
	std::shared_ptr<Scene> CreatePlane(float width = 1.0f, float height = 1.0f, bool reverse_winding = false);
