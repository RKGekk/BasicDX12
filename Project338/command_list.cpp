...
std::shared_ptr<Scene> CommandList::LoadSceneFromFile(const std::wstring& file_name, const std::function<bool(float)>& loading_progress) {
	auto scene = std::make_shared<Scene>();

	if (scene->LoadSceneFromFile(*this, file_name, loading_progress)) {
		return scene;
	}
...
std::shared_ptr<Scene> CommandList::CreateCube(float size, bool reverse_winding) {
	float s = size * 0.5f;
...
std::shared_ptr<Scene> CommandList::CreateSphere(float radius, uint32_t tessellation, bool reversWinding) {
	...
	for (size_t i = 0; i <= verticalSegments; i++) {
		...
		for (size_t j = 0; j <= horizontalSegments; j++) {
			...
			auto position = DirectX::XMVectorScale(normal, radius);
	...
	for (size_t i = 0; i < verticalSegments; i++) {
		for (size_t j = 0; j <= horizontalSegments; j++) {
			...
			indices.push_back(static_cast<IndexCollection::value_type>(i * stride + nextJ));
			indices.push_back(static_cast<IndexCollection::value_type>(nextI * stride + j));
			indices.push_back(static_cast<IndexCollection::value_type>(i * stride + j));

			indices.push_back(static_cast<IndexCollection::value_type>(nextI * stride + nextJ));
			indices.push_back(static_cast<IndexCollection::value_type>(nextI * stride + j));
			indices.push_back(static_cast<IndexCollection::value_type>(i * stride + nextJ));
		}
	}
...
std::shared_ptr<Scene> CommandList::CreateCylinder(float radius, float height, uint32_t tessellation,
	...
	for (size_t i = 0; i <= tessellation; i++) {
		...
		indices.push_back(static_cast<IndexCollection::value_type>(i * 2 + 1));
		indices.push_back(static_cast<IndexCollection::value_type>((i * 2 + 2) % (stride * 2)));
		indices.push_back(static_cast<IndexCollection::value_type>(i * 2));

		indices.push_back(static_cast<IndexCollection::value_type>((i * 2 + 3) % (stride * 2)));
		indices.push_back(static_cast<IndexCollection::value_type>((i * 2 + 2) % (stride * 2)));
		indices.push_back(static_cast<IndexCollection::value_type>(i * 2 + 1));
	}
...
std::shared_ptr<Scene> CommandList::CreateCone(float radius, float height, uint32_t tessellation, bool reverseWinding) {
	...
	for (size_t i = 0; i <= tessellation; i++) {
		...
		DirectX::XMVECTOR normal = DirectX::XMVector3Cross(GetCircleTangent(i, tessellation), DirectX::XMVectorSubtract(topOffset, pt));
		...
		indices.push_back(static_cast<IndexCollection::value_type>((i * 2 + 1) % (stride * 2)));
		indices.push_back(static_cast<IndexCollection::value_type>((i * 2 + 3) % (stride * 2)));
		indices.push_back(static_cast<IndexCollection::value_type>(i * 2));
	}
...
std::shared_ptr<Scene> CommandList::CreateTorus(float radius, float thickness, uint32_t tessellation,
	...
	for (size_t i = 0; i <= tessellation; i++) {
		...
		for (size_t j = 0; j <= tessellation; j++) {
			...
			auto position = DirectX::XMVectorScale(normal, thickness / 2);
			auto textureCoordinate = DirectX::XMVectorSet(u, v, 0, 0);
...
std::shared_ptr<Scene> CommandList::CreatePlane(float width, float height, bool reverseWinding) {
	...
	VertexCollection vertices = {
		Vertex(DirectX::XMFLOAT3(-0.5f * width, 0.0f, 0.5f * height), DirectX::XMFLOAT3(0.0f, 1.0f, 0.0f), DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f)),  // 0
		Vertex(DirectX::XMFLOAT3(0.5f * width, 0.0f, 0.5f * height), DirectX::XMFLOAT3(0.0f, 1.0f, 0.0f), DirectX::XMFLOAT3(1.0f, 0.0f, 0.0f)),   // 1
		Vertex(DirectX::XMFLOAT3(0.5f * width, 0.0f, -0.5f * height), DirectX::XMFLOAT3(0.0f, 1.0f, 0.0f), DirectX::XMFLOAT3(1.0f, 1.0f, 0.0f)),  // 2
		Vertex(DirectX::XMFLOAT3(-0.5f * width, 0.0f, -0.5f * height), DirectX::XMFLOAT3(0.0f, 1.0f, 0.0f), DirectX::XMFLOAT3(0.0f, 1.0f, 0.0f))  // 3
	};
	// clang-format on
